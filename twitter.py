...
def search_by_location_and_word(twitter_api,latitude,longitude,max_range,keyword, num_results=200):
        '''
        ...
        while result_count <  num_results:
                ...
                for result in query["statuses"]:
                            ...
                            if result["geo"]:
                                    ...
                                    try:                                    
                                        location_name = result["entities"]["name"]
                                        location_screen_name = result["entities"]["screen_name"]
                                    except KeyError:
                                        location_name = "NOT FOUND"
                                        location_screen_name  = "NOT FOUND"
                                    # we can also try to collect the url to 4sq checkin:
                                    try:
                                        foursqr_url = result["entities"]["urls"][0]["expanded_url"]  
                                    except KeyError:
                                    ...
                                    user_location = result["user"]["location"] # to check that it is Chicago
                                    user_desc = result["user"]["description"] # if we want to analyze the users
                                    created_at = result["created_at"] # for time of checkin
...
if __name__=='__main__':
    ...
    # search API:
    twt_list = search_by_location_and_word(twitter_api,latitude,longitude,max_range,keyword, num_results=200)
